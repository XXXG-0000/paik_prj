	...
	if ("post".equalsIgnoreCase(request.getMethod())) {
	    ...
	    try{
		    ...
		    if("이미 탈퇴된 계정입니다.".equals(tempPassword)){
		    	session.invalidate();
	    		out.print("<script>alert('이미 탈퇴된 계정입니다.'); location.href='http://paiks.sist.co.kr/index.jsp';</script>");
				return;
	    	...
	    	if(tempPassword != null && tempPassword.length() % 16 == 0){
	    		...
	    		try{
	    			...
	    			e.printStackTrace();
			    	session.invalidate();
		    		out.print("<script>alert('문제가 발생했습니다. 다시 시도해주세요.'); location.href='http://paiks.sist.co.kr/index.jsp';</script>");
	    			return;
	    		}//try~ca
